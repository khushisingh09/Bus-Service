...
const BusSearch = () => {
  ...
  <div className="flex flex-col space-y-4 md:flex-row md:space-y-0 justify-between items-center md:space-x-4">
    ...
    <button
      ...
      className="bg-blue-700 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg w-full md:w-auto self-stretch md:self-end"
    >
      Search
    </button>
  </div>
  ...
  {searchResults.length > 0 ? (
    searchResults.map((bus) => (
      <div
        ...
        style={{ height: "auto" }} // Make height auto for mobile responsiveness
      >
        {/* Bus Image */}
        <img
          src={bus.image}
        ...
        {/* Bus Details */}
        <div className="flex flex-col md:flex-row justify-between p-4 w-full space-y-4 md:space-y-0 md:space-x-4">
          {/* Left Section - Operator and Info */}
          ...
          <div className="w-full md:w-1/3 flex flex-col justify-between">
            <div className="flex items-center justify-between">
              ...
              <div className="text-center">
                <p className="text-gray-600 text-lg">{bus.departureTime}</p>
          ...
          <div className="w-full md:w-1/3 flex flex-col justify-between items-end">
            <div className="text-right">
              <h3 className="font-bold text-xl">Price</h3>
              <p className="line-through text-gray-500">₹{bus.oldPrice}</p>
              <p className="text-red-500 font-bold text-2xl">₹{bus.newPrice}</p>
            </div>
    ...
    <p className="text-gray-500">No buses found for the selected route.</p>
      ...
      {selectedBus && (
  <div
    className={`fixed inset-0 bg-black bg-opacity-50 flex justify-center sm:justify-end items-center transition-all duration-300 ${
      isDialogOpen ? "opacity-100" : "opacity-0"
    }`}
  >
    <div
      className={`bg-white p-4 rounded-lg shadow-lg w-full sm:w-4/5 md:w-3/5 lg:w-2/5 transition-transform duration-300 transform ${
    ...
    >
      <div className="relative">
        {/* Close Button */}
        <button
          className="absolute top-2 right-2 text-gray-500 hover:text-gray-800 text-2xl bg-red-200 p-2 rounded-full z-10"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center my-6">
          <div className="text-center mb-4 sm:mb-0">
            <p className="font-bold text-lg">{selectedBus.departureTime}</p>
            <p className="text-gray-700 text-md font-bold">{selectedBus.from}</p>
          </div>

          <div className="relative w-full sm:w-1/2 mx-2">
            <hr className="border-gray-300" />
          ...
          <div className="text-center mb-4 sm:mb-0">
            <p className="font-bold text-lg">{selectedBus.arrivalTime}</p>
            <p className="text-gray-700 text-md font-bold">{selectedBus.to}</p>
          </div>
        </div>

        {/* Price and Confirmation Button */}
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
          <div className="text-right mb-4 sm:mb-0">
            <h3 className="font-bold text-lg">
              Price: <span className="text-red-500">₹{selectedBus.newPrice}</span>
            </h3>
            <p className="line-through text-gray-500 text-sm">₹{selectedBus.oldPrice}</p>
          </div>
        ...
        {/* Ticket Booking, QR Code, and Any Query */}
        <div className="flex flex-col sm:flex-row items-center justify-between p-3 border rounded-lg bg-gray-100 mb-2">
